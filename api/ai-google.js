...
function formatarAssento(conteudo, numeroOpcao = '') {
    try {
        ...
        // NÃO adicionar assento se não foi mencionado
        return '';
...
function aplicarPosProcessamento(resultado, conteudoOriginal, parcelamentoSelecionado) {
    try {
        ...
        // Corrigir formatação de passageiros (incluindo "adultoss" com duplo S)
        resultado = resultado.replace(/(\d+)\s*(Adulto+s*|Criança|Bebê)/gi, (match, num, tipo) => {
            ...
            // Corrigir "adultoss" ou qualquer variação
        ...
        // Formatar parcelamento que não foi processado
        if (resultado.includes('Entrada de R
...
7. Formatar passageiros: 04 adultos + 01 criança (NÃO "adultoss")
8. Manter placeholders como instruído
9. Adicionar (+1) para chegadas no dia seguinte
10. Terminar com (v2.8)

FORMATAÇÃO OBRIGATÓRIA:
- Parcelamento: 💳 Em até Xx, sendo a primeira de R$ X, mais Yx de R$ Y s/ juros no cartão
- Bagagem SEM despachada: ✅ Inclui 1 item pessoal + 1 mala de mão de 10kg
- Bagagem COM despachada: ✅ Inclui 1 item pessoal + 1 mala de mão de 10kg + 1 bagagem despachada de 23kg
- Assento (só se mencionado): 💺 Inclui pré reserva de assento
- Reembolso: 🏷️ Não reembolsável ou 🏷️ Reembolsável conforme regras do bilhete
...
console.log('========================================');)) {
            resultado = resultado.replace(
                /Entrada de R\$\s*([\d.,]+)\s*\+\s*(\d+)x\s+de\s+R\$\s*([\d.,]+)[^\\n]*/gi,
                (match, entrada, numParcelas, valorParcela) => {
                    const totalParcelas = parseInt(numParcelas) + 1;
                    return `💳 Em até ${totalParcelas}x, sendo a primeira de R$ ${entrada}, mais ${numParcelas}x de R$ ${valorParcela} s/ juros no cartão`;
                }
            );
        }
        
        // Formatar bagagem não processada
        resultado = resultado.replace(
            /^(Com bagagem|Sem bagagem)[^\\n]*/gm,
            (match) => {
                if (match.toLowerCase().includes('com bagagem')) {
                    const extras = [];
                    if (match.toLowerCase().includes('pré reserva') || match.toLowerCase().includes('pre reserva')) {
                        extras.push('💺 Inclui pré reserva de assento');
                    }
                    return '✅ Inclui 1 item pessoal + 1 mala de mão de 10kg + 1 bagagem despachada de 23kg' + 
                           (extras.length > 0 ? '\n' + extras.join('\n') : '');
                } else {
        ...
        // Formatar reembolso não processado
        resultado = resultado.replace(
            /^(Não reembolsável|Reembolsável)/gm,
            (match) => {
                if (match.toLowerCase().includes('não')) {
                    return '🏷️ Não reembolsável';
                } else {
