// ⚙️ api/modules/config.js - v8.1 - CORREÇÃO FINAL DE EXPORTAÇÕES E SINTAXE
// CORREÇÃO: Removido 'export' duplicado de todas as constantes/funções.
// CORREÇÃO: Removida chave 'selecaoModelo' duplicada e aninhada incorretamente.
...
const CVC_CONFIG = {
  ...
  DETECTION: {
    ...
    ORDEM_PRIORIDADE: [
      'cruzeiro', 'multitrecho', 'multiplas_opcoes_3', 'multiplas_opcoes_2',
      'multiplas_companhias_internacionais', 'aereo_somente_ida', 'pacote_completo',
      'aereo_conexao_detalhada', 'aereo_nacional_simples'
    ]
  }
};
...
const VALIDATION_CONFIG = {
  ...
  CVC: {
    ...
    TIPOS_VALIDOS: [
      'Aéreo Nacional', 'Aéreo Internacional', 'Cruzeiros', 'Hotéis',
      'Pacotes', 'Multitrechos', 'Rankings', 'Dicas'
    ]
  }
};
...
function validateConfig() {
  ...
  const errors = [];
  const warnings = [];
  if (!API_CONFIG.OPENAI.API_KEY) warnings.push("OpenAI API Key não configurada - usando modo mock");
  else if (API_CONFIG.OPENAI.API_KEY.length < VALIDATION_CONFIG.API_KEY_MIN_LENGTH) errors.push("OpenAI API Key parece inválida");
  if (!API_CONFIG.ANTHROPIC.API_KEY) warnings.push("Anthropic API Key não configurada - funcionalidade limitada");
  else if (API_CONFIG.ANTHROPIC.API_KEY.length < VALIDATION_CONFIG.API_KEY_MIN_LENGTH) errors.push("Anthropic API Key parece inválida");
  if (!SERVER_CONFIG.PORT) errors.push("Porta do servidor não definida");
  if (SERVER_CONFIG.PORT < 1000 || SERVER_CONFIG.PORT > 65535) errors.push("Porta do servidor fora do intervalo válido");
  if (API_CONFIG.OPENAI.TIMEOUT < 5000) warnings.push("Timeout da OpenAI muito baixo (< 5s)");
  if (API_CONFIG.ANTHROPIC.TIMEOUT < 5000) warnings.push("Timeout da Anthropic muito baixo (< 5s)");
  if (warnings.length > 0) warnings.forEach(warning => console.warn(`⚠️ ${warning}`));
  if (errors.length > 0) {
...
function getConfigStatus() {
  return {
    ...
    server: { port: SERVER_CONFIG.PORT, host: SERVER_CONFIG.HOST, cors_origin: SERVER_CONFIG.CORS_ORIGIN },
    apis: {
      openai: { configured: !!API_CONFIG.OPENAI.API_KEY, model: API_CONFIG.OPENAI.MODEL, max_tokens: API_CONFIG.OPENAI.MAX_TOKENS },
      anthropic: { configured: !!API_CONFIG.ANTHROPIC.API_KEY, model: API_CONFIG.ANTHROPIC.MODEL, max_tokens: API_CONFIG.ANTHROPIC.MAX_TOKENS }
    },
    features: { mock_mode: DEV_CONFIG.MOCK_MODE, debug_mode: DEV_CONFIG.ENABLE_DEBUG, verbose_logging: DEV_CONFIG.VERBOSE_LOGGING, log_requests: LOG_CONFIG.LOG_REQUESTS },
    templates: { total_tipos: Object.keys(CVC_CONFIG.TEMPLATES).length, tipos_disponiveis: Object.values(CVC_CONFIG.TEMPLATES) },
    destinos: { aeroportos_nacionais: Object.keys(DESTINOS_CONFIG.aeroportos_nacionais).length, aeroportos_internacionais: Object.keys(DESTINOS_CONFIG.aeroportos_internacionais).length, destinos_populares: DESTINOS_CONFIG.populares_nacionais.length + DESTINOS_CONFIG.populares_internacionais.length }
  };
}
...
function getConfig(path, defaultValue = null) {
  try {
    ...
    let current = { API_CONFIG, SERVER_CONFIG, LOG_CONFIG, CVC_CONFIG, VALIDATION_CONFIG, RESPONSE_CONFIG, SECURITY_CONFIG, DEV_CONFIG, DESTINOS_CONFIG };
...
function updateConfig(path, value) {
  ...
  try {
    ...
    if (path.includes('PORT') && (value < 1000 || value > 65535)) { console.error("❌ Porta inválida"); return false; }
    if (path.includes('TIMEOUT') && value < 1000) { console.error("❌ Timeout muito baixo"); return false; }
...
function getAeroportoInfo(codigo) {
  const aeroporto = DESTINOS_CONFIG.aeroportos_nacionais[codigo] || DESTINOS_CONFIG.aeroportos_internacionais[codigo];
  if (!aeroporto) return { codigo, nome: codigo, tipo: 'desconhecido', encontrado: false };
  return { codigo, nome: aeroporto, tipo: DESTINOS_CONFIG.aeroportos_nacionais[codigo] ? 'nacional' : 'internacional', encontrado: true };
}
